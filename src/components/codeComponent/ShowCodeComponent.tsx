...
export default function CodeViewer({
  ...
  const handleMouseEnter = (event: React.MouseEvent, line: LineData) => {
    const { clientY, clientX } = event;
    setAlertPosition({ top: clientY, left: clientX });
    setHoveredLine(line);
  ...
  return (
    <section className="w-full max-w-[88%] mx-auto my-[60px]">
            ...
            {sourceData?.[0]?.sources?.map((line: any) => {
              ...
              return (
                <div
                  ...
                  className="flex gap-14"
                  onMouseEnter={(event) => {
                    setIsHovered(line.utLineHits === 0 || line.lineHits === 0);
                    handleMouseEnter(event, line);
                  }}
      ...
      {isHovered && hoveredLine && alertPosition && (
        <section
          style={{
            position: "absolute",
            top: alertPosition.top + 10,
            left: alertPosition.left + 10,
            zIndex: 1000,
          }}
        >
          <Alert className="max-w-md mx-auto">
            <AlertDescription>
              ...
              <div className="mt-2">
                <strong>Author:</strong> {hoveredLine.scmAuthor || "Unknown"}
              </div>
              <div className="mt-2">
                <strong>Revision:</strong>{" "}
                {hoveredLine.scmRevision || "Unknown"}
              </div>
              <div className="mt-2">
                <strong>Committed on:</strong>{" "}
                {new Date(hoveredLine.scmDate).toLocaleString()}
              </div>
            </AlertDescription>
          </Alert>
        </section>
